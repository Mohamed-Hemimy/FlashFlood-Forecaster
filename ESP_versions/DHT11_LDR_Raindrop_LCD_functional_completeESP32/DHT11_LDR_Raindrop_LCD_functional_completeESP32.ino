...
bool displayMode = true;                // Toggle display mode

// Variables to store sensor values
float temperature = 0.0;
float humidity = 0.0;
int rain_amount = 0;
int flashFloodLikelihood = 0;
int LDRValue = 0;
...
void loop() {
  ...
  if (currentTime - lastMeasurementTime >= 2000) {
    ...
    // Read sensor data
    rain_amount = analogRead(RAIN_ANALOG);
    ...
    flashFloodLikelihood = calculateFloodLikelihood(temperature, humidity);
  ...
  if (currentTime - lastDisplayTime >= 1000) {
    ...
    lcd.clear();
    if (displayMode) {
      // First display: Temperature, Humidity, and LDR
      lcd.setCursor(0, 0);
      lcd.print("T:");
      ...
      // Second display: Rain amount and Likelihood
      lcd.setCursor(0, 0);
      lcd.print("Rain:");
